...
class BLF_ILC:
    ...
    def __init__(self, rate, run_time):
        ...
        self.result = {'error': [], 'input': [], 'time': [], 'angle': [np.pi / 3-self.max_value for i in range(2)],
                       ...
                       'orientation': [np.pi / 3-self.max_value for i in range(2)], 'd_orientation': [], 'total_torque': [],
    ...
    def control_law(self, orientation, step):
        ...
        # epsilon = (0.4 - 0.1) * np.exp(-0.5 * t) + 0.1
        epsilon = 0.2
        # epsilon = 0.2
        ...
        last_phi = [self.project(self.last_list_input['hat_phi'][step][i], self.phi_start) for i in range(3)]
        last_d = self.project(self.last_list_input['hat_d'][step], self.d_start)
        # adaptive law
        hat_phi = [last_phi[i] + self.gamma * error * xi[i] for i in range(3)]
        hat_d = last_d + self.lamda * dv_de
        # control law
        u = dv_de + self.Gamma * d_error + self.B / (self.B ** 2) * (
            np.dot(np.mat(hat_phi), np.mat(xi).T)[0, 0]+desired['acc']) + hat_d
        # u = last_input + dv_de + self.Gamma * d_error
        # u=Ve^2*sin(alpha)
        self.Ve = np.sqrt(2 * (4 + np.abs(u)))
        self.alpha = np.arcsin(u / (2 * (4 + np.abs(u))))
        self.last_input = u
        # save data
        self.result['input'].append(u)
        self.result['hat_phi'].append(hat_phi)
